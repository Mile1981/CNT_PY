...
class pninno(db.Model):
    __tablename__ = 'pninno'

    id = db.Column(db.Integer, primary_key=True)
    relacionPeso = db.Column(db.String(45), nullable=False)
    ...
    def get_data(cls, _id):
     ...
     @classmethod
    def getPesoEstaturaPorHistoria(cls, _id):
    ...
    def insert(cls, dataJson):
        query = (                 
                relacionPeso = dataJson["relacionPeso"], 
                idHistoriaClinica = dataJson["idHistoriaClinica"],                
            )
        pninno.guardar(query)
        if query.id:                            
            return  query.id 
        return  False
    ...
    def update_data(cls, _id, dataJson):
        try:
            ...
            query = cls.query.filter_by(id=_id).first()
            if query:
                if "relacionPeso" in dataJson:
                    query.relacionPeso = dataJson["relacionPeso"]
